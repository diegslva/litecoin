...
LITECOIND=${LITECOIND:-$SRCDIR/medicoind}
LITECOINCLI=${LITECOINCLI:-$SRCDIR/medicoin-cli}
LITECOINTX=${LITECOINTX:-$SRCDIR/medicoin-tx}
LITECOINQT=${LITECOINQT:-$SRCDIR/qt/medicoin-qt}

[ ! -x $LITECOIND ] && echo "$LITECOIND not found or not executable." && exit 1

# The autodetected version git tag can screw up manpage output a little bit
MDIVER=($($LITECOINCLI --version | head -n1 | awk -F'[ -]' '{ print $6, $7 }'))
